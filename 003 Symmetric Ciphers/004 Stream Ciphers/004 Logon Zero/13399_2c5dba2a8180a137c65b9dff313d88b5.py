...
class CFB8:
    ...
    def decrypt(self, ciphertext):
        IV = ciphertext[:16] # weakness
        ct = ciphertext[16:]
        cipher = AES.new(self.key, AES.MODE_ECB) # in ecb mode
        pt = b''
        state = IV
...
class Challenge():
    def __init__(self):
        ...
        self.password = urandom(20) # 20 digit password
        self.password_length = len(self.password)
        self.cipher = CFB8(urandom(16)) # 16 digit seed
    ...
    def challenge(self, your_input):
        ...
        if your_input['option'] == 'reset_password':
            ...
            print(f"token hex: {token.hex()}, len: {len(token.hex())}")
