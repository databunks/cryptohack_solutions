...
msg_enc_bytes = bytes.fromhex(msg_enc)
flag_enc_bytes = bytes.fromhex(flag_enc)
state = bytes_to_words(xor(msg[:64], msg_enc_bytes[:64]))

for i in range(10):
    rev_inner_block(state)

key = words_to_bytes(state[4:12])
print(ChaCha20().decrypt(flag_enc_bytes, key, bytes.fromhex(iv2)))
